      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md p-6">
            <h2 className="text-lg font-bold mb-5">📋 Raise Purchase Request</h2>
            <div className="flex flex-col gap-4">
              <div>
                ...
                {selectedItem && (
                  <div className="mt-1.5 p-2 rounded-lg flex items-center justify-between" style={{ background: parseFloat(selectedItem.current_qty) <= selectedItem.min_stock_level ? '#ef444412' : '#10b98112', border: `1px solid ${parseFloat(selectedItem.current_qty) <= selectedItem.min_stock_level ? '#ef4444' : '#10b981'}` }}>
                    <span className="text-[10px] font-semibold" style={{ color: parseFloat(selectedItem.current_qty) <= selectedItem.min_stock_level ? '#ef4444' : '#10b981' }}>
                      {parseFloat(selectedItem.current_qty) <= selectedItem.min_stock_level ? '⚠ LOW STOCK' : '✓ OK'}
                    </span>
                    <span className="text-[11px] font-bold">{parseFloat(selectedItem.current_qty).toFixed(1)} {selectedItem.unit_of_measure} in stock</span>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <textarea className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring resize-none"
                  rows={3} placeholder="e.g. Stock critically low, production halted. Need within 2 days..."
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={handleCreate} disabled={saving || !form.item_id || !form.requested_qty || !form.reason}
                className="flex-1 h-10 rounded-lg text-sm font-bold disabled:opacity-50" style={{ background: '#f97316', color: '#fff' }}>
